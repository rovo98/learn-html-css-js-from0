...
//
let arr1 = [1, 3, 5, 7, 9]
console.log(arr1.reduce((x, y) => x + y)) // 求和
...
let n = [1, 3, 5, 7, 9]
console.log(n.reduce((x, y) => x * 10 + y)) // 13579

// parseInt navie implementation
function myParseInt (str) {
  let nums = str.split('').map(Number)
  return nums.reduce((x, y) => x * 10 + y)
}
...
result2 = testArr4.filter((element, index, self) => self.indexOf(element) === index)
...
console.log(testArr5.sort((x, y) => x - y)) // 顺序排序
console.log(testArr5.sort((x, y) => y - x)) // 倒序排序
